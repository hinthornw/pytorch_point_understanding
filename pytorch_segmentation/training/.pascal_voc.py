...
class PascalVOCSegmentation(data.Dataset):
    ...
    def __init__(self,
        ...
        assert mask_type in _MASKTYPE, "mask_type {} error. Must be one of {}".format(mask_type, list(_MASKTYPE)) 
        ...
        self.point_annotations = get_pascal_object_part_points(os.path.join(self.root, self.PASCAL_POINT_FOLDER_NAME),
                self.PASCAL_POINT_FILE_NAME)
    ...
    def __getitem__(self, index):
        ...
        # _point_target, _weights = get_point_mask(point_annotations, self.mask_type, np.array(_semantic_target).shape)
        # _point_target, _weights = Image.fromarray(_point_target), Image.fromarray(_weights)
        _point_target = get_point_mask(point_annotations, self.mask_type, np.array(_semantic_target).shape)

        _point_target = Image.fromarray(_point_target)

        print("_point_target values:\t{}".format(np.unique(_point_target)))

        if self.joint_transform is not None:
            #_img, _semantic_target, _point_target, _weights  = self.joint_transform([_img, _semantic_target, _point_target, _weights])
            _img, _semantic_target, _point_target = self.joint_transform([_img, _semantic_target, _point_target])
